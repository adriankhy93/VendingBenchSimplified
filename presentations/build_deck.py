...
W, H = 960, 540
C = dict(bg='F5F8F7', paper='FFFFFF', ink='173A42', muted='557079', teal='008C7B',
         lime='CAEFA1', line='DCE6E2', pale='E8F3ED', dark='102F37', white='FFFFFF', blue='5E8DCD')
FONTS = Path(os.environ.get('VENDING_SLIDE_FONT_DIR', '/usr/share/fonts/truetype/dejavu'))
for key, file in [('regular','DejaVuSans.ttf'),('bold','DejaVuSans-Bold.ttf'),('mono','DejaVuSansMono.ttf')]:
...
class Deck:
    def __init__(self, output):
        ...
        self.prs.core_properties.subject = 'Participant briefing for a pre-generated test environment'
    ...
    def rect(self, x,y,w,h,fill='paper',radius=0,stroke=None):
        shape=self.slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE if radius else MSO_SHAPE.RECTANGLE,Pt(x),Pt(y),Pt(w),Pt(h))
        if radius:
            shape.adjustments[0]=min(.15,radius/min(w,h))
        shape.fill.solid();shape.fill.fore_color.rgb=RGBColor.from_string(self.color(fill))
        if stroke:
            shape.line.color.rgb=RGBColor.from_string(self.color(stroke));shape.line.width=Pt(.7)
        else:
        ...
        self.pdf.roundRect(x,H-y-h,w,h,radius,fill=1,stroke=1 if stroke else 0)
        return shape
    ...
    def line(self,x1,y1,x2,y2,color='line',width=1):
        ...
        s.line.color.rgb=RGBColor.from_string(self.color(color));s.line.width=Pt(width)
        self.pdf.setStrokeColor(HexColor('#'+self.color(color)));self.pdf.setLineWidth(width)
        self.pdf.line(x1,H-y1,x2,H-y2)

    def text(self, text,x,y,w,h,size=18,color='ink',bold=False,mono=False,leading=1.27):
        font='mono' if mono else 'bold' if bold else 'regular'
        lines=wrap(text,font,size,w)
        used=size+(len(lines)-1)*size*leading
        if used>h+0.1:
            raise ValueError(f'Slide {self.page}: text overflow ({used:.1f}>{h}): {text}')
        if min(x,y)<0 or x+w>W+.1 or y+h>H+.1:
        ...
        for i,line in enumerate(lines):
            ...
            p.text=line;p.font.name='DejaVu Sans Mono' if mono else 'DejaVu Sans'
        ...
        self.pdf.setFont(font,size);self.pdf.setFillColor(HexColor('#'+self.color(color)))
        for i,line in enumerate(lines):
            self.pdf.drawString(x,H-y-size*.82-i*size*leading,line)
        self.text_boxes.append(dict(slide=self.page,text=text,x=x,y=y,w=w,h=h,lines=len(lines),size=size))
        return used

    def start(self,spec,index):
        self.page=index;self.slide=self.prs.slides.add_slide(self.prs.slide_layouts[6])
        self.dark=spec['layout'] in ('cover','score','checklist')
        self.rect(0,0,W,H,'dark' if self.dark else 'bg')
        self.text(spec['section'],52,28,810,18,10,'lime' if self.dark else 'teal',True)
        if spec['layout']!='cover':
            self.text(spec['title'],52,65,856,90,31,'white' if self.dark else 'ink',True,leading=1.16)
        self.slide.notes_slide.notes_text_frame.text=spec['notes']+'\n\nImplementation references: '+', '.join(spec['sources'])

    def finish(self,spec):
        if spec['layout']!='cover':
            self.rect(52,460,856,43,'193F45' if self.dark else 'pale',radius=7)
            self.text(spec['takeaway'],66,472,827,28,12,'lime' if self.dark else 'ink',bold=True,leading=1.15)
        self.line(52,517,908,517,'32515A' if self.dark else 'line',.65)
        self.text('VENDING BENCH  /  PARTICIPANT BRIEFING',52,524,670,10,7.5,'A6C1C6' if self.dark else 'muted')
        self.text(f'{self.page:02}',881,523,27,12,9,'lime' if self.dark else 'teal',True)
        self.pdf.showPage()
    ...
    def columns(self,spec,top=174,height=264):
        for i,col in enumerate(spec['columns']):
            ...
            self.text(col['title'],x+22,top+22,374,35,12,'teal' if i==0 else 'muted',True)
            y=top+62
    ...
    def steps(self,spec,loop=False):
        steps=spec['steps'];gap=17;w=(856-gap*(len(steps)-1))/len(steps)
        top=185
        for i,step in enumerate(steps):
            ...
            self.text(step['title'],x+18,top+76,w-36,33,22 if not loop else 18,'ink',True)
            self.text(step['body'],x+18,top+120,w-36,105 if not loop else 73,17 if not loop else 15,leading=1.33)
            if i<len(steps)-1:
                self.text('›',x+w+4,top+98,14,30,25,'teal',True)
        if loop:
            self.line(76,417,260,417,'teal',2)
            self.line(705,417,884,417,'teal',2)
            self.text('Repeat while running and within budget',275,402,420,32,15,'teal',True)

    def stats(self,spec,budget=False):
        items=spec['stats'];gap=16;w=(856-gap*(len(items)-1))/len(items)
        for i,item in enumerate(items):
            x=52+i*(w+gap)
            self.rect(x,177,w,125,'paper',radius=10)
            self.text(item['value'],x+17,193,w-34,49,32 if budget else 27,'teal',True)
    ...
    def table(self,spec):
        ...
        widths=[220,636] if count==2 else ([165,421,270] if self.page==11 else [195,320,341])
        x=52;top=167
        self.rect(x,top,856,34,'ink',radius=5)
        left=x
        for header,width in zip(spec['headers'],widths):
            self.text(header.upper(),left+13,top+10,width-25,18,12,'white',True)
            left+=width
        rows=spec['rows'];height=242/len(rows)
        for i,row in enumerate(rows):
            ...
            for j,(text,width) in enumerate(zip(row,widths)):
                size=15.5
                self.text(text,left+13,y+12,width-25,height-14,size,'teal' if j==0 else 'ink',bold=j==0,leading=1.19)
                left+=width

    def render(self,spec,index):
        self.start(spec,index);layout=spec['layout']
        if layout=='cover':
            ...
            for row in range(4):
                for col in range(3):
                    x=694+col*60;y=168+row*47
                    self.rect(x,y,48,36,'294E54',radius=5)
                    self.rect(x+17,y+8,15,22,'lime' if (row+col)%3==0 else '71A99B',radius=4)
            self.rect(696,374,120,16,'dark',radius=4)
            self.rect(838,371,20,22,'teal',radius=4)
        elif layout=='columns':self.columns(spec)
        elif layout=='steps':self.steps(spec)
        elif layout=='loop':self.steps(spec,True)
        elif layout=='stats':self.stats(spec)
        elif layout=='budget':self.stats(spec,True)
        elif layout=='table':self.table(spec)
        elif layout=='score':
            self.text('FINAL SCORE',55,169,800,19,11,'lime',True)
            self.text(spec['formula'],55,205,845,79,25,'white',True,leading=1.35)
            for i,label in enumerate(spec['example']):
                x=52+i*174
                self.rect(x,326,160,98,'lime' if i==4 else '22464D',radius=9)
                parts=label.split(' ')
                amount=' '.join(parts[:2]) if parts[0] in ('+','−','=') else parts[0]
                desc=' '.join(parts[2:]) if parts[0] in ('+','−','=') else ' '.join(parts[1:])
                self.text(amount,x+15,342,136,40,27,'ink' if i==4 else 'white',True)
            ...
            pts=[(114+i*10,245+124*(1-math.exp(-i/8))) for i in range(27)]
            for a,b in zip(pts,pts[1:]):self.line(*a,*b,'teal',3)
            y=173
            for item in spec['bullets']:
            ...
            self.text(spec['code'],70,225,537,204,14,'white',mono=True,leading=1.53)
            y=184
            for item in spec['bullets']:
            ...
            self.text('http://localhost:8080',659,177,225,23,12,'white',mono=True)
            ...
            for i,(label,key) in enumerate([('REQUEST','request'),('RESPONSE EXCERPT','response')]):
                x=52+i*438
                self.rect(x,164,418,279,'ink',radius=10)
                self.text(label,x+18,182,380,19,10,'lime',True)
                self.text(spec[key],x+18,215,382,220,13.5,'white',mono=True,leading=1.37)
    ...
    def save(self):
        ...
        self.pdf.save()
        (ROOT/'layout-check.json').write_text(json.dumps(dict(slides=self.page,text_boxes=len(self.text_boxes),bounds_checked=True),indent=2)+'\n')


def main():
    spec=json.loads((ROOT/'participant-briefing.json').read_text())
    notes=['# Participant briefing — speaker notes', '', '21 slides: 18-slide core briefing plus 3 reference slides. Approximate speaking time: 20 minutes plus questions.', '']
    for i,slide in enumerate(spec['slides'],1):
        notes += [f"## {i:02}. {slide['title'].replace(chr(10),' ')}", '', slide['notes'], '', 'Sources: '+', '.join(slide['sources']), '']
    (ROOT/'participant-speaker-notes.md').write_text('\n'.join(notes))
    deck=Deck(ROOT/'participant-briefing')
    for i,slide in enumerate(spec['slides'],1):deck.render(slide,i)
    deck.save()
