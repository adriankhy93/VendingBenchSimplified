...
class SavedEnvironment(EnvironmentConfig):
    ...
    def complete_quotes(self):
        if len(self.scenario.supplier_quotes) != len(self.scenario.products) * len(self.scenario.suppliers):
            raise ValueError('saved environments must contain every resolved supplier/product quote')
        return self
...
def environment_path(directory, name):
    ...
    if not isinstance(name, str) or not re.fullmatch(NAME_PATTERN, name):
        raise ValueError('environment name must use letters, digits, hyphens, or underscores')
...
def generate(config_path, name, directory='environments'):
    ...
    resolved = tuple(PairQuote(supplier_id=q.supplier_id, product_id=q.product_id, category=q.category,
                              minimum_cents=q.minimum, listed_cents=q.listed) for q in quotes.values())
    scenario = Scenario.model_validate(config.scenario.model_dump() | dict(supplier_quotes=resolved))
...
def main():
    ...
    try:
        if args.command == 'generate':
            print(generate(args.config, args.name, args.directory))
        else:
            for path in sorted(Path(args.directory).glob('*.json')):
                saved, digest = load_environment(args.directory, path.stem)
                print(f'{saved.name}\t{len(saved.scenario.products)} products\t{len(saved.scenario.suppliers)} suppliers\tseed={saved.seed}\t{digest[:12]}')
