...
class Engine:
    def __init__(self, config: Scenario, seed: int):
        ...
        self.slots = {
            f"r{r}s{s}": Slot(f"r{r}s{s}")
            for r in range(1, config.machine_rows + 1)
            for s in range(1, config.slots_per_row + 1)
        }
        ...
        self.revenue = self.cogs = self.fees_assessed = self.fees_paid = (
            self.discounts
    ...
    def validate(self, action, payload):
        ...
        for field, catalog in (
            ...
            ("supplier_id", {s for s, _ in self.quotes}),
            ("slot_id", self.slots),
        ):
    ...
    def bounds(self, pid):
        ...
        return (
            ref * self.config.min_price_percent + 99
        ) // 100, ref * self.config.max_price_percent // 100

    def balance(self):
        return dict(
            cash_cents=self.cash,
            machine_cash_cents=self.machine_cash,
            fee_debt_cents=self.debt,
        )
    ...
    def inventory(self):
        return {
            p: {
                ...
                "acquisition_cost_cents": sum(
                    l.quantity * l.unit_cost_cents for l in lots
                ),
                "lots": [asdict(l) for l in lots],
            }
            for p, lots in self.storage.items()
        }
    ...
    def machine(self):
        return dict(
            ...
            units_sold=dict(self.sold),
            current_day_sales=dict(self.today),
        )
        ...
        return dict(
            products=[
                dict(
                    product_id=p.id,
                    name=p.name,
                    reference_price_cents=p.reference_price_cents,
                    min_price_cents=self.bounds(p.id)[0],
                    max_price_cents=self.bounds(p.id)[1],
                )
                for p in self.products.values()
                if product_id in (None, p.id)
            ],
            suppliers=[s.id for s in self.config.suppliers],
            quotes=[
                q.public()
                for q in self.quotes.values()
                if product_id in (None, q.product_id)
            ],
        )

    def observe(self):
        return dict(
            scenario_version=self.config.version,
            **self.catalog(),
            **self.balance(),
            **self.machine(),
            ...
            rules=dict(
                ...
                machine_rows=self.config.machine_rows,
                slots_per_row=self.config.slots_per_row,
                tick_minutes=self.config.tick_minutes,
                quantity_cap=self.config.quantity_cap,
    ...
    def apply(self, action, args):
        ...
        if action == "make_offer":
            pid, sid, qty, price = (
                args[k]
                for k in ("product_id", "supplier_id", "quantity", "unit_price_cents")
            )
    ...
    def advance(self, minutes):
        ...
        remaining = minutes
        while remaining:
            to_midnight = 1440 - self.minute % 1440
            elapsed = min(remaining, to_midnight)
            self.minute += elapsed
            remaining -= elapsed
            if elapsed != to_midnight:
                continue
            day = self.minute // 1440
            multiplier = self.config.day_multipliers[
                (day - 1) % len(self.config.day_multipliers)
            ]
            for index, (pid, p) in enumerate(self.products.items()):
                ...
                count = demand.sample(
                    ...
                    index,
                    day - 1,
                    demand.expected_daily(p, price, multiplier),
                )
                available = sum(
                    s.quantity for s in self.slots.values() if s.product_id == pid
                )
                sold = min(available, count)
                self.stockouts[pid] += count - sold
                if count:
                    self.private_events.append(
                        dict(day=day, product_id=pid, demand=count, sold=sold)
                    )
                units_remaining = sold
                for slot in self.slots.values():
                    if slot.product_id != pid or not units_remaining:
                        continue
                    take = min(slot.quantity, units_remaining)
            ...
            events.append(
                dict(
                    type="day",
                    day=day,
                    sales=dict(self.today),
                    fee_assessed_cents=fee,
                    fee_paid_cents=paid,
                    arrears_paid_cents=arrears,
    ...
    def execute(self, action, payload):
        ...
        minutes = (
            1440 - self.minute % 1440
            if action == "end_day"
            else self.config.durations[action]
        )
        ...
        if action == "observe":
            ...
            result = self.machine()
        response = dict(
            action_id=f"act_{self.action_count:04}",
            state=self.state,
            sim_time=dict(
                day=self.minute // 1440 + 1, minute_of_day=self.minute % 1440
            ),
            elapsed_minutes=self.minute - start,
            result=result,
            events=events,
            metrics=dict(**self.balance(), units_sold=sum(self.sold.values())),
    ...
    def summary(self, complete=True):
        return dict(
            ...
            scenario_version=self.config.version,
            score=score(self),
            simulated_minutes=self.minute,
            completed_days=self.minute // 1440,
            metrics=dict(
                revenue_cents=self.revenue,
                cost_of_goods_sold_cents=self.cogs,
                fees_assessed_cents=self.fees_assessed,
                fees_paid_cents=self.fees_paid,
                fees_unpaid_cents=self.debt,
                units_sold=dict(self.sold),
                purchases=self.purchase_count,
                negotiated_discounts_cents=self.discounts,
