...
def create_app(runs_dir="runs", service_dir=None, pi_sessions_dir=None):
    app = FastAPI(title="Vending run viewer")
    store = RunStore(runs_dir, service_dir, pi_sessions_dir)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="View recorded vending runs in your browser"
    )
    parser.add_argument("--runs-dir", default="runs")
    parser.add_argument(
        "--service-dir",
        help="Trusted pre-deletion summaries; defaults to RUNS_DIR/service",
    )
    parser.add_argument(
        "--pi-sessions-dir", help="Pi JSONL sessions; defaults to RUNS_DIR/pi-sessions"
    )
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8080)
    args = parser.parse_args()
    print(
        f"Run viewer: http://{args.host}:{args.port}  |  {Path(args.runs_dir).resolve()}",
        flush=True,
    )
    uvicorn.run(
        create_app(args.runs_dir, args.service_dir, args.pi_sessions_dir),
        host=args.host,
        port=args.port,
    )
