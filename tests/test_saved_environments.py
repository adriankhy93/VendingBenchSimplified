...
from test_harness import LocalClient
from test_environment_config import custom_scenario


def saved(tmp_path, name='custom'):
    config = EnvironmentConfig(seed=42,scenario=custom_scenario(scenario_id='smoke-v1',max_days=1,runtime_seconds=20))
...
@pytest.mark.parametrize('name',['../escape','a/b','/tmp/foo','name.json','', '..'])
...
def test_selected_creation_preserves_definition_and_isolation(tmp_path):
    ...
    with TestClient(create_app(registry)) as client:
        ...
        assert client.post('/env',json=options|dict(environment_sha256='0'*64)).status_code == 409
        assert client.post('/env',json=dict(environment_name='missing')).status_code == 404
        assert client.post('/env',json=dict(environment_name='../custom')).status_code == 422
        initial=client.post(f'/env/{a}/observe',json={}).json()
        assert initial['result']['cash_cents'] == 900 and len(initial['result']['slots']) == 4
        for secret in ('seed','supplier_quotes','minimum_cents','category_weights','environment_sha256'):
            assert secret not in json.dumps(initial)
        client.post(f'/env/{a}/end_day',json={})
        assert registry.status(a)['state'] == 'ended' and registry.status(b)['state'] == 'running'


def test_runner_uses_saved_seed_runtime_and_logs_definition(tmp_path):
    _,directory=saved(tmp_path)
    registry=Registry(artifact_dir=tmp_path/'private',environments_dir=directory)
    client=LocalClient(create_app(registry))
    try:
        output,summary=run(RunConfig(environment_name='custom',environments_dir=str(directory),
                                     artifact_dir=str(tmp_path/'runs'),seed=999,runtime_seconds=1),client=client)
        assert summary['complete'] and summary['evaluator']['seed'] == 42
        effective=json.loads((output/'config.json').read_text())
        assert effective['runtime_seconds'] == 20 and effective['max_days'] == 1
        assert effective['environment_definition']['scenario']['starting_cash_cents'] == 900
        assert summary['terminal']['score']['score_cents'] == 900
...
def test_resolved_quotes_and_default_seed_market_match(tmp_path):
    ...
    assert Engine(original.scenario,17).quotes == Engine(definition.scenario,17).quotes
